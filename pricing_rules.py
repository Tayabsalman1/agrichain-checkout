...
class PricingRules:
    def __init__(self, unit_price:int, offers:list[Offer]=[]):
    ...
    def calculate_price(self, unit_count):
        ...
        offers = sorted(self.unit_offers, key=lambda x: x.quantity, reverse=True)

        #Applying Offers to get offer related prices for applicable quantities
        remaining_unit_count = unit_count
        offer_unit_price = 0
        for offer in offers:
            offer_quantity_multiple = remaining_unit_count // offer.quantity
            remaining_unit_count = remaining_unit_count % offer.quantity
            offer_unit_price += offer_quantity_multiple *offer.price
